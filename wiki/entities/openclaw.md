...
OpenClaw 是一个开源的自主 AI Agent 平台，运行在本地基础设施上（Windows/macOS/Linux）。由 Peter Steinberger（PSPDFKit 创始人）于 2025 年 11 月创建。采用本地优先（local-first）哲学——所有数据、API keys、运行行为留在本地，无云端依赖。MIT 协议开源。

在多 Agent 协作层面，采用"交响乐团"式的异步编排模式，核心是 subagent 系统，支持事件驱动的异步协作、运行时引导（Steer）、持久化和恢复。

GitHub 增长：72 小时 60,000 stars → 3 个月 247,000 stars。
