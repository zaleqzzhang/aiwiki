...
**马匹隐喻**（NxCode）：模型是马——快速、强大但不知方向；Harness 是缰绳马具——约束、引导、反馈；工程师是骑手——提供方向，不亲自跑。

**四动词框架**（Martin Fowler 扩展）：
1. **Constrain**：约束 Agent 能做什么（架构边界、依赖规则）
2. **Inform**：告知 Agent 应该做什么（上下文工程、文档）
3. **Verify**：验证 Agent 做对了没（测试、Linting、CI）
4. **Correct**：纠正 Agent 的错误（反馈循环、自修复）
...
- LangChain：仅改 Harness（不换模型），Terminal Bench 2.0 从 Top 30 跳到 Top 5（52.8% → 66.5%）
- OpenAI Codex 团队：100 万行生产代码，零人类手写，5 个月完成（传统估算的 1/10 时间）
- Stripe Minions：每周 1000+ merged PRs，开发者只参与发任务和审查合并
- OpenCode Hashline：仅改编辑工具（Harness 组件），Grok Code Fast 从 6.7% → 68.3%——**10x 改进**，目前最强单一 Harness 组件影响的经验证据

**熵管理**（Harness 第三大被低估的支柱）：
- AI 生成的代码库随时间积累熵——文档偏移、命名约定分裂、死代码堆积
- 解法：周期性清理 Agent（文档一致性、约束违规扫描、模式强制、依赖审计）
- 按日/周/事件触发运行

**约束反直觉效应**：约束解空间反而让 Agent 更高效。无约束时浪费 token 探索死胡同，有约束时更快收敛到正确方案。

**"可撕裂性"（Rippability）**：好的 Harness 组件应该能随模型能力提升而移除。过度工程化的控制流会被下一代模型打破。

## 两大设计流派（Tom Foster）

| 流派 | 代表 | 核心信念 |
|------|------|---------|
| **Reasoning-First** | Claude Code, Mistral Vibe 2.0 | 信任模型，最小脚手架，投资上下文压缩 |
| **Environment-First** | Codex CLI, Gemini CLI | 投资环境可读性和机械强制 |

**40% 上下文利用率阈值**：长上下文研究发现，超过约 40% 窗口利用率后 Agent 可靠性显著下降。这解释了 Reasoning-First 流派的 aggressive 压缩策略。
...
| 级别 | 适用场景 | 搭建时间 | 核心内容 |
|------|---------|---------|---------|
| Level 1 | 个人开发者 | 1-2 小时 | CLAUDE.md + pre-commit hooks + 测试套件 + 清晰目录结构 |
| Level 2 | 3-10 人团队 | 1-2 天 | + AGENTS.md + CI 架构约束 + 共享 Prompt 模板 + Agent PR 审查清单 |
| Level 3 | 工程组织 | 1-2 周 | + 自定义中间件（循环检测、推理优化）+ 可观测性 + 熵管理 Agent + Harness A/B 测试 + 升级策略 |
...
- [[sources/harness-engineering-complete-guide|Harness Engineering: Complete Guide]] - 马匹隐喻、四动词框架、三大支柱、三级落地框架、OpenAI/Stripe/LangChain 实践案例
- [[sources/harness-architecture-comparison|Why Agent Harness Architecture is Important]] - 五款 Harness 对比、Reasoning-First vs Environment-First 两大流派、Hashline 10x 证据
...
- ~~三根支柱的优先级如何排序？~~ → NxCode 三级落地框架给出参考：先 Context（文档）→ 再 Constraints（CI）→ 最后 Entropy Management
- 如何量化 Harness 的 ROI？（LangChain Terminal Bench 和 Princeton 数据提供了初步度量方式）
- ~~不同规模团队应该如何选择实现路径？~~ → 三级落地框架解答：Level 1（个人）→ Level 2（小团队）→ Level 3（组织）
- Harness 组件的"可撕裂性"如何评估？何时该移除一个中间件？
