...
- **OpenCode Hashline 的发现过程**：OpenCode 团队分析编辑失败的 trace，发现瓶颈不是模型推理能力，而是"模型意图到文件修改的机械接口"——模型复制原文时出错。这个洞察直接催生了 Hashline 方案（行哈希引用），带来 10x 性能提升。**没有 trace 分析，就不会发现真正的瓶颈在哪。**
