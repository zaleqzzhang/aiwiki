...
- **Meta-Harness 的 6 倍差距**：同一个底模，只改 Harness 代码，SWE-Bench 上性能差 6 倍。OpenCode 的 Hashline 更极端——10 倍改进只改编辑接口。这证明了 Harness 层改进的巨大潜力。
- **两大流派的不同 Harness 层策略**：Reasoning-First（Claude Code、Mistral Vibe）投资上下文压缩和简洁主循环；Environment-First（Codex CLI、Gemini CLI）投资仓库可读性和机械强制。两条路径都在 Harness 层改进，但哲学不同——前者信任模型，后者信任环境。
