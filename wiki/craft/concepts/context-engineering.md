...
**核心挑战是 Context Rot（上下文衰减）**：随着 token 数量增加，模型准确召回信息的能力下降。Chroma 的数据显示：关键内容放在上下文窗口中间位置时，性能下降超过 30%。长上下文研究进一步量化了这个阈值：**上下文窗口利用率超过约 40% 后，Agent 可靠性显著下降**——这解释了为什么 Reasoning-First 流派（Claude Code、Mistral Vibe）强调 aggressive 压缩。Context Engineering 的目标不是"把所有相关信息塞进去"，而是策展**最小可行的高信号 token 集**。
...
- **OpenClaw 时间衰减记忆**：衰减系数 = e^(-λ × 天数)，λ = ln(2) / 半衰期（默认 30 天）。30 天减半，60 天剩 1/4，90 天剩 1/8。这是 Context Engineering 在记忆维度的精细化——不只管"现在看什么"，还管"过去的哪些记忆还值得注入"。
